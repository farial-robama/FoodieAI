      ...
      <div className="flex gap-2 flex-wrap">
        {["all", "confirmed", "pending", "cancelled"].map((s) => (
          <button
            ...
            style={
              filter === s ? { backgroundColor: "var(--color-primary)" } : {}
            }
          >
            {s} (
            {s === "all"
            ...
            <thead>
              <tr className="border-b border-stone-200 dark:border-stone-800">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-xs font-medium text-stone-500 uppercase tracking-wide"
                  >
                    {h}
                  </th>
                ))}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1.5">
                        {b.status === "pending" && (
                          <>
                            <button
                              ...
                              className="text-xs px-2.5 py-1 rounded-lg text-white cursor-pointer font-medium"
                              style={{
                            ...
                            <button
                              ...
                              className="text-xs px-2.5 py-1 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 cursor-pointer font-medium"
                            >
                              ✕ Cancel
                            </button>
                          </>
                        ...
                        {b.status === "confirmed" && (
                          <button
                            onClick={() => updateStatus(b._id, "cancelled")}
                            className="text-xs px-2.5 py-1 rounded-lg bg-red-100 text-red-600 hover:bg-red-200 cursor-pointer"
                          >
                            Cancel
                          </button>
                        )}
                        {b.status === "cancelled" && (
                          <button
                            ...
                            className="text-xs px-2.5 py-1 rounded-lg bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400 hover:bg-stone-200 cursor-pointer"
                          >
                            Restore
                          </button>
                        )}
