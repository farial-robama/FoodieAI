...
// import {
//   LayoutDashboard, Calendar, Star, Heart, User,
//   BarChart2, UtensilsCrossed, Users, Settings, X,
// } from "lucide-react";
...
//             )}
//             style={pathname === href ? { backgroundColor: "var(--color-primary)" } : {}}
//           >
//             <Icon size={16} />
//             {label}
//           </Link>
  ...
  useEffect(() => {
    ...
    const tryFetch = async () => {
      try {
        const res  = await fetch(`/api/users?clerkId=${userId}`);
        if (!res.ok) {
          if (attempts < 5) { attempts++; setTimeout(tryFetch, 1000); }
          return;
        }
        const data = await res.json();
